...
Latest credential dry-run: `2026-06-09T22:36:34.967Z`
...
The local ignored template is present and intentionally empty as of the latest dry-run; `npm run apply:bettafish-credentials -- --dry-run` correctly reports no usable credentials until real values are filled and now emits `nextSteps` with the exact template path and apply commands.
