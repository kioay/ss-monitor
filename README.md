...
During full sync, the deployment applies a small set of production compatibility patches after cloning upstream: `/api/config` responses redact secret-like values, MindSpider generates MediaCrawler DB config from runtime environment variables instead of embedding credentials, and MediaCrawler's default DB config reads the same environment variables. MediaCrawler profile/data/temp directories remain runtime state and are not part of the reproducible code patch set.
