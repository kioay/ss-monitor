...
- Production BettaFish can be deployed as a full runtime tree with `npm run sync:bettafish-full`. Keep `.env`, browser profiles, cookies, crawler output, downloaded media, generated reports, caches, and training datasets out of the sync unless the user explicitly asks for a specific class of runtime state.
