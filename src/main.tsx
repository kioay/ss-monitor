  ...
  const loadInspiration = React.useCallback(
    async (refresh = false) => {
      ...
      try {
        ...
        const payload = (await response.json()) as InspirationResponse;
        if (latestRequestRef.current !== requestId) return;
        setData(payload);
      } catch (reason) {
        if (latestRequestRef.current !== requestId) return;
...
function InspirationAssetCard({ asset, highlightQuery }: { asset: InspirationAsset; highlightQuery: string }) {
  ...
  return (
    <article className={`inspiration-card ${asset.category}`}>
      ...
      <div className="inspiration-card-body">
        ...
        <p><HighlightText text={asset.reason} query={highlightQuery} /></p>
        <div className="pill-row">
          <span>{inspirationCategoryLabel(asset.category)}</span>
          {asset.visualTags.slice(0, 4).map((tag) => (
            <span key={tag}><HighlightText text={tag} query={highlightQuery} /></span>
          ))}
...
function inspirationCategoryLabel(category: InspirationCategoryFilter) {
  if (category === "weapon_skin") return "武器皮肤";
  if (category === "character_skin") return "角色皮肤";
  if (category === "general_reference") return "综合参考";
  return "全部素材";
}
...
ReactDOM.createRoot(document.getElementById("root")!).render(
  <React.StrictMode>
    {window.location.pathname.startsWith("/inspiration") ? <InspirationPage /> : <App />}
  </React.StrictMode>
);
