(() => {
  ...
  function assertSubmittedTurnStillRunning(detail, options = {}) {
    ...
    if (!["failed", "failed_to_schedule", "cancelled", "canceled"].includes(status)) return;
  ...
  function createAgentAppClient(options) {
    ...
    async function waitForResult(input = {}) {
      const timeoutMs = Math.max(1, Number(input.timeoutMs || 180000));
      ...
      throw new Error("Timed out waiting for WDCloud Agent App session result.");
    }
